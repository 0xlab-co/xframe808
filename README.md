...
- 前景 / 後景圖片不必是固定像素尺寸，只要符合所選比例即可，程式會自動正規化到固定畫布
...
3. 點「選擇檔案...」選擇後景底圖、前景套框，或兩者都選
4. 點「選擇資料夾...」選擇放商品圖的資料夾
5. 點「選擇資料夾...」選擇輸出資料夾
6. 預覽區會自動顯示第一張合成結果
7. 需要時調整 X/Y 位移或縮放
8. 點「開始套框」開始批次處理

> 前景 / 後景圖片只要符合所選比例即可，不需要剛好是 1080×1080 或 1080×1920。若比例不符，預覽與批次會直接提示錯誤。
>
...
├── main.py                 # 進入點
├── core/
│   └── compositor.py       # 比例 preset、圖層驗證與圖片合成邏輯
├── tests/
│   └── test_compositor.py  # 核心合成測試
├── ui/
│   ├── main_window.py      # 主視窗與預覽流程
│   └── worker.py           # 背景批次執行緒
├── scripts/
